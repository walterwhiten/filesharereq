...
from config import REQUEST_CHANNEL, FORCE_CHANNEL, FORCE_CHANNEL2
import pyrogram.utils
...
class Bot(Client):
    ...
    async def start(self):
        ...
        if REQUEST_CHANNEL:
            try:
                link_a = (await self.create_chat_invite_link(chat_id=REQUEST_CHANNEL, creates_join_request=True)).invite_link 
                self.link_one = link_a                                     
            except Exception as a:
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot can't Export Invite link from Force Sub Channel!")
                self.LOGGER(__name__).warning(f"Please Double check the REQUEST_CHANNEL value and Make sure Bot is Admin in channel with Invite Users via Link Permission, Current Force Sub Channel Value: {CHANNEL_ONE}")
                self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/Anime_X_Hunters for support")
                sys.exit()
        if FORCE_CHANNEL:
            try:
        ...
        if FORCE_CHANNEL2:
            try:
                link = (await self.get_chat(FORCE_CHANNEL2)).invite_link
                if not link:
                    await self.export_chat_invite_link(FORCE_CHANNEL2)
                    link = (await self.get_chat(FORCE_CHANNEL2)).invite_link
        ...
        try:
            ...
            self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/https://t.me/Anime_X_Hunters for support")
            sys.exit()

        self.set_parse_mode(ParseMode.HTML)
        self.LOGGER(__name__).info(f"Bot Running..!\n\nCreated by \nhttps://t.me/Its_Oreki_Hotarou")
